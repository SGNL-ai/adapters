...
func (a *Adapter) RequestPageFromDatasource(
	...
	req := &Request{
		...
		Customer:              request.Config.Customer,
	...
	resp, err := a.GoogleWorkspaceClient.GetPage(ctx, req)
	if err != nil {
		return framework.NewGetPageResponseError(err)
	}
	...
	parsedObjects, parserErr := web.ConvertJSONObjectList(
		...
		web.WithDateTimeFormats(
			[]web.DateTimeFormatWithTimeZone{
				// Google Workspace does not provide a standardized datetime format.  However, all the
				// observed date-time formats (i.e. User.lastLoginTime) are in RFC3339 format.
